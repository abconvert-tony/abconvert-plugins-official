...
4. **Data path.** The Shopify MCP's `run-analytics-query` tool on the connected store (`switch-shop` first if several are connected). It reads the store's production analytics, so get a clear yes before the first query.

Timezone comes from `get-shop-info`. ShopifyQL day rows are store-local; hour rows are UTC. Convert once and use store-local everywhere, including the merchant's own dates.
